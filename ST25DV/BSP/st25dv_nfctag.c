...
NFCTAG_StatusTypeDef BSP_NFCTAG_Init(void)
{
  ...
  if (!NfctagInitialized) {
    ...
    if ((nfctag_id == I_AM_ST25DV04) || (nfctag_id == I_AM_ST25DV16) || (nfctag_id == I_AM_ST25DV64)) {
        NfctagInitialized = 1;
...
uint32_t BSP_NFCTAG_GetByteSize(void)
{
  ...
  if (Nfctag_Drv == NULL) {
    // printk("Nfctag_Drv is null\n");
  }

  if (Nfctag_Drv->pData == NULL) {
    // printk("Nfctag_Drv->pData is null\n");
  }

  ((NFCTAG_ExtDrvTypeDef *)Nfctag_Drv->pData)->ReadMemSize(&mem_size);
  // printk("BSP_NFCTAG_GetByteSize - success \n");
